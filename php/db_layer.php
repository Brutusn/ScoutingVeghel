...
* @return The id of the corresponding huurder and reservering or [-1, -1] if none present
*/
function getVerhuringFromConfirm($key) {
  ...
  if($stmt_gv = $mysqli->prepare("CALL GetVerhuringFromConfirm(?)")){
    ...
    $stmt_gv->execute();
    $stmt_gv->bind_result($vid, $hid, $rid);
    while ($stmt_gv->fetch()) {
      $verhuring_id = $vid;
      $reservering_id = $rid;
      $huurder_id = $hid;
  ...
  return [$verhuring_id, $huurder_id, $reservering_id];
}
